...
# ── Step 4：取消跟踪个人脚本（已在 .gitignore，但可能已被 track）────────────
info "取消跟踪个人脚本..."
for PERSONAL in scripts/build.sh scripts/client.sh scripts/publish.sh; do
  if git ls-files --error-unmatch "$PERSONAL" 2>/dev/null; then
    git rm --cached "$PERSONAL"
    success "  已取消跟踪：$PERSONAL"
  fi
done

# ── Step 5：用 bootstrap 模板替换个人配置文件 ─────────────────────────────────
