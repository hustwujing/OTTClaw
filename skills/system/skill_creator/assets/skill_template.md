...
**第二步：处理**
- 调用 skill(action=read_asset, skill_id=xxx, asset_name=yyy) 读取参考资料
- 调用 skill(action=run_script, skill_id=xxx, script_name=yyy, args=[...]) 执行脚本
...
调用 `skill(action=read_asset)` 获取本技能的写作规范：
- skill_id: my_skill
- asset_name: style_guide.md
...
调用 `skill(action=read_asset, skill_id=csv_analyzer, asset_name=report_template.md)` 获取标准报告格式。
生成报告时严格遵循该模板。
...
6. 如果 SKILL.md 引用了 `skill(action=read_asset)`，对应的资源文件**必须通过 `skill(action=write, skill_id=..., content=..., sub_path="assets/<文件名>")` 创建**。
