...
**Actions**: `launch`, `close`, `navigate`(url), `snapshot`, `screenshot`(fullPage?), `click`(ref), `type`(ref,text), `select`(ref,values), `hover`(ref), `scroll`(ref|deltaY), `press_key`(key,ref?), `wait`(selector|timeoutMs), `evaluate`(script), `tabs`, `tab_open`(url?), `tab_close`(targetIdx), `save_cookies`(cookieName), `load_cookies`(cookieName), `list_cookies`

**Key params**: `url`, `ref` (from snapshot, e.g. `e3`), `text`, `key`, `values`, `selector`, `script`, `deltaY`(default 500), `fullPage`, `targetIdx`, `cookieName`(letters/digits/-/_ only), `timeoutMs`(default 10000), `visible`(bool, `launch` only — see login flow)
